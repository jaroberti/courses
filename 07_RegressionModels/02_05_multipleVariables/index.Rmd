...
nosim <- 1000
#define 3 independent normal regressors
x1 <- rnorm(n)
x2 <- rnorm(n)
x3 <- rnorm(n)
#simulate data where the outcome Y only depends on x1 and the standard error.  Then we are going to look at the model that only includes x1, the model that includes x1 & x2, and the model that includes x1,x2, and x3. The variance inflation occurs on the actual standard errors and not the estimated standard error.
...
#what happens? nothing big really happens to the variance inflation (i.e., the output values are very similar) because the x2 and x3 variables have nothing to do with x1.... but what if these values (x2 and x3) are dependent on x1? (see below)
```
...
nosim <- 1000
x1 <- rnorm(n)
x2 <- x1/sqrt(2) + rnorm(n) /sqrt(2) #dependent on x1 
x3 <- x1 * 0.95 + rnorm(n) * sqrt(1 - 0.95^2)#dependent on x1 
betas <- sapply(1 : nosim, function(i){
  y <- x1 + rnorm(n, sd = .3)
  c(coef(lm(y ~ x1))[2], 
    coef(lm(y ~ x1 + x2))[2], 
...
round(apply(betas, 1, sd), 5)
#now we see a big difference in inflation across all three variables.  This means that when you put highly correlated variables into the model you will inflate the variance more.  You need to be careful adding in highly correlated variables into the dataset. For example, if you included height and weight in a model (which are usually highly correlated) you will inflate your standard error of the model.  
```
...
```{r}
library(car)
#fit the model with all the variables included.
fit <- lm(Fertility ~ . , data = swiss)
vif(fit)
sqrt(vif(fit)) #I prefer sd (i.e., this)

#what are the results showing us?
#for vif(fit) 
#Agriculture is 2.28, so what does that mean? It means that the standard error for agriculture effect is double what it would be if it were orthogonal to the other regressors.  Examination and Education are highly related to the outcome (fertility), and hence they actually have the largest VIF, but that's OK because they are important and correlated to fertility rates.
```
...
#here we can test if the addition of extra terms is necessary. we see a listing of the models with degrees of freedom (Res.df); Root sum of squares (RSS); DF = extra degrees of freedom relative to previous fit. the p-values show that the inclusion of the extra variables seems to be necessary based on the F statistics and p values...
